---
title:     OctoCop Director for Docker (OCDD)
subtitle:  Traffic re-directory for Docker
author:    kamranazeem
comments:  true
tags:
  - Docker
  - Tools
  - Automated
avatar:     /images/stories/octocop-director-for-docker/ocdd-temp-logo.jpg
nav-weight: 5
published: false
---

<strong>Imagine you didn't need IT to help you configure DNS names and you could automate the DNS names setup and avoid proxying your services in Docker containers. Yoy can do all of that with OctoCop Director for Docker (OCDD) and you avoid disturbing your IT department and to wait on them.
<!--break-->

OctoCop DD or OCDD for short is a tool to make the life of IT administrators easy in situations when CoDers (like us) want to setup a CoDe (Continuous Delivery) toolstack, which could include Jenkins and Artifactory.

OCDD is not a proxy, and is not a load balancer. It is merely a traffic re-director - thus the name.

# The itch
It (the itch - of-course) started during one of Praqma's super-cool CoDe camps, when a colleague of mine in Copenhagen described me a situation and wanted to have a solution for it. As a CoDer he had to provide (and setup) a tool stack  in some company which was supposed to run Jenkins, Artifactory, Gitlab and a few other software tools. The problem was that these tools were running on a server as Docker containers, and asking IT department to assign a IP address to these services and updating their DNS was proving to be a slow process. Also there were often times when a new service would be setup on the same server (as a Docker container), and again IT had to be involved and they were often very business. The idea was to have something in our control and not bother anybody from the IT department. It was also possible that some of the services would possibly have a port conflict with other ports on the same server, but changing port numbers for the conflicting services was not an option as it would prove to be to costly to maintain the host-container mapping. Further it could require users to do configuration on their side, e.g. non default ssh configuration for git servers, which we also think is not optimal.
...
* Setup a new infrastructure IP on host's network interface for each container running on this server.
...
* You can use root to setup OCDD. You can also use a normal user to set up OCDD. In that case the user needs to have password-less sudo access.
* In addition to having Docker / Docker-Engine and Docker-Compose, you need to have git (to download this repo - of-course), and [jq](https://stedolan.github.io/jq/download/) for json parsing.
...
## 1. Get the repo on the new server
...
## 2. Start the example application (Jenkins + Artifactory)
...
## 4. Run the OCDD script in normal mode:
To see the real magic happening, run OCDD in normal mode. Simply execute `./ocdd.sh` and you get:
...
![OCDD listing running services on simple webpage of our domain.](OCDD-web-list.png)
...
It also leaves you the freedom to change port on the application as you like. In production we would obviously move jenkins from port 8080 to port 80, and we would do that with Artifactry also. With OCDD we can have both on port 80 and we don't need proxies.

So here is the screenshot that shows the Jenkins web page:

![OCDD sucessfully gave us access to Jenkins on the DNS name.](OCDD-jenkins.png)


## Access the Artifactory service using it's own DNS name

![OCDD successfully gave us access to Artifactory on the DNS name. ](OCDD-artifactory.png)


## C-Advisor
Just for fun, and to get some important information about your containers, a C-Advisor container is included with OCDD:

![OCDD includes C-Advisor container showing important information about your containers.](OCDD-cadvisor.png)


# Summary
OctoCopDD completely soothes this itch of how to have control on our CoDe server or services, without routinely bothering (read: bugging) IT department of an organization. The project is accessible through [https://github.com/praqma/octocopdd](https://github.com/praqma/octocopdd) , and following our agile principle of *Continuous Improvement*, it is work in progress - though production ready. I hope you will enjoy deploying and using it as much as I did while developing it.
